...
const findAppointId = async (req, res) => {
    let pointId = req.query.pid;
    let docId = req.query.did;
    Appointment.find((err, result) => {
        let d = '', p = '';
        Doctor.find((err, Dr) => {
            d = Dr.filter(v => v.userId == docId)[0]._id
            let data = result.filter(v => {
                // console.log(String(v.doctorId) == String(d), v.patientId, pointId)
                return String(v.doctorId) == String(d) && String(v.patientId) == pointId
            });
            // console.log(data)
            res.send(data[0]._id)
